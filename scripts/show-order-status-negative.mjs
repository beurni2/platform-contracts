#!/usr/bin/env node
// Raw demonstration (WO-1.0 evidence): a sixth order-status string must
// REFUSE at parse. Exits non-zero on the refusal.
...
const order = {
  ...
  status: 'shipped', // NOT one of the five E1 statuses
  timestamps: { createdAt: '2026-07-09T10:00:00Z' },
};

const result = OrderSchema.safeParse(order);
if (result.success) {
  console.error('BUG: a sixth order status parsed — the enum asserts nothing');
  process.exit(0); // exit 0 signals the gate FAILED to catch it
}
console.error(`order status 'shipped' REFUSED at parse (as required):`);
